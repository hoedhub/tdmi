...
export const actions: Actions = {
    default: async ({ request, locals }) => {
        ...
        const existingUser = await db.query.usersTable.findFirst({ where: eq(usersTable.username, username) });
        if (existingUser) {
        ...
        if (muridIdStr) {
            ...
            if (isNaN(parsedId)) { return fail(400, { ...formState, message: 'Murid ID tidak valid.' }); }
            muridId = parsedId;
        }
