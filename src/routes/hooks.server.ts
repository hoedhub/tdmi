...
function isSKRedirect(error: any): error is { status: number; location: string } {
    return typeof error === 'object' && error !== null &&
        typeof error.status === 'number' && error.status >= 300 && error.status <= 399 &&
...
export const handle: Handle = async ({ event, resolve }) => {
    ...
    if (!sessionId) {
        ...
        if (!isPublicPath && !isApiPath) {
            console.log(`[HOOKS] No session ID, not public/api, redirecting from ${url.pathname} to /login`);
            redirect(302, '/login');
        }
    } else {
        ...
        try {
            ...
            if (session) {
                ...
                console.log("[HOOKS] Session NOT validated (validateSession returned no session). Clearing cookie.");
                const sessionCookie = lucia.createBlankSessionCookie();
                event.cookies.set(sessionCookie.name, sessionCookie.value, { path: ".", ...sessionCookie.attributes });
                if (!isPublicPath && !isApiPath) {
                    console.log(`[HOOKS] Invalid session (after validation), not public/api, redirecting from ${url.pathname} to /login`);
                    redirect(302, '/login');
                }
            }
            ...
            if (!isPublicPath && !isApiPath) {
                console.log(`[HOOKS] Error during session validation, not public/api, redirecting from ${url.pathname} to /login`);
                redirect(302, '/login');
            }
        }
    }
    ...
    if (event.locals.session && (url.pathname === '/login' || url.pathname === '/register')) {
        console.log(`[HOOKS] User HAS SESSION and is on login/register page. Redirecting from ${url.pathname} to /member.`);
        redirect(303, '/member'); // Or 303
    }

    console.log(`[HOOKS] Proceeding to resolve for ${url.pathname}. locals.session set: ${!!event.locals.session}`);
    try {
        ...
        if (isSKRedirect(error)) {
            console.log(`[HOOKS] Propagating redirect from resolve: ${error.status} to ${error.location} for ${url.pathname}`);
            throw error;
        }
