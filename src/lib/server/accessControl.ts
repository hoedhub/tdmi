import {
    ...
    getRolePermissions,
    isSubRole
} from './accessControlDB'; // <-- Menggunakan file baru
import { db } from '$lib/drizzle';
import { usersTable, muridTable, deskelTable, kecamatanTable, kokabTable, propTable } from '$lib/drizzle/schema';
...
async function isWithinTerritoryScope(userId: string, resourceDeskelId: number): Promise<boolean> {
    ...
    if (!result) {
        console.warn(`Could not determine territory for user ${userId} or resource ${resourceDeskelId}.`);
        return false;
    }
...
export async function userHasPermission(
    ...
    if (isLevel3User && requiresTerritoryCheck) {
        ...
        if (!withinScope) {
            console.log(`User ${userId} denied access to resource in deskelId ${resource.deskelId} (out of scope).`);
            return false;
        }
    }
    ...
    if (isWritePermission && hasTargetRole) {
        ...
        const checks = userAssignedRoles.map(userRoleId => isSubRole(userRoleId, resource.targetRoleId!));
        ...
        if (!canWriteSubRole) {
            console.log(`User ${userId} denied write access on role ${resource.targetRoleId} (not a sub-role).`);
            return false;
        }
    }
