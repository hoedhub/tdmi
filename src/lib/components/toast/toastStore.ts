...
function generateId(): string {
    return crypto.randomUUID ? crypto.randomUUID() : `toast-${Math.random().toString(36).substring(2, 9)}-${Date.now()}`;
}
...
export function show(options: ToastUserOptions): string {
    ...
    const duration = options.duration === 0 ? Infinity : (options.duration || storeConfig.defaultDuration);
        ...
        toasts.map(toast => {
            if (toast.id === id) {
                ...
                if (newOptions.duration !== undefined && newOptions.progress === undefined) {
                    progress = newOptions.duration > 0 && newOptions.duration !== Infinity && DEFAULT_PROGRESS;
                }
