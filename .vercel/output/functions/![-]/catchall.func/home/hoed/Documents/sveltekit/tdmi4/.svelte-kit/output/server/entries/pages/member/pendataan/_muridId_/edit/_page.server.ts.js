...
import { g as getPublicFileUrl, d as deleteFile, u as uploadFile } from "../../../../../../chunks/cloudinary.js";
const load = async ({ locals, params }) => {
  ...
  }).from(muridTable).leftJoin(deskelTable, eq(muridTable.deskelId, deskelTable.id)).leftJoin(kecamatanTable, eq(deskelTable.idKecamatan, kecamatanTable.id)).leftJoin(kokabTable, eq(kecamatanTable.idKokab, kokabTable.id)).leftJoin(propTable, eq(kokabTable.idProp, propTable.id)).where(eq(muridTable.id, muridId)).get();
  if (!muridData) {
  ...
  default: async ({ request, locals, params }) => {
    ...
    try {
      const updatedValues = {
        ...
        marhalah: parseInt(formData.get("marhalah")?.toString() || "1"),
