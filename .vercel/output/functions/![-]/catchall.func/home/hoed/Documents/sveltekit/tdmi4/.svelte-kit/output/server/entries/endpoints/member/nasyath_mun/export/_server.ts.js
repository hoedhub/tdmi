...
import { eq, like, or, gte, lte, and, asc, desc, getTableColumns } from "drizzle-orm";
...
const POST = async ({ request, locals }) => {
  ...
  try {
    ...
    const dataToExport = await db.select({
      ...getTableColumns(nasyathTable),
      murid: {
        nama: muridTable.namaArab
      }
    }).from(nasyathTable).leftJoin(muridTable, eq(nasyathTable.muridId, muridTable.id)).where(whereClause).orderBy(...orderByClauses).all();
    let periodString = "";
    const monthNames = [
      "Januari",
      "Februari",
    ...
    if (periodType === "bulan" && dateInfo) {
      ...
      periodString = `Export_${(/* @__PURE__ */ new Date()).toISOString().split("T")[0]}`;
    }
