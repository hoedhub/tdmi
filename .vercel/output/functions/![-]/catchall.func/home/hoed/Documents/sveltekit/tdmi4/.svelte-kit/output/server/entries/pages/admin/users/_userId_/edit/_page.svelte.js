...
function _page($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    let { data, form } = $$props;
    let isLoading = false;
    let username = data.userToEdit.username;
    let selectedRoles = data.userToEdit.assignedRoles;
    let isActive = data.userToEdit.active;
    let muridId = data.userToEdit.muridId === null ? "" : String(data.userToEdit.muridId);
    run(() => {
    ...
    data.userToEdit.assignedRoles.map((roleId) => data.allAvailableRoles.find((r) => r.id === roleId)?.name).filter(Boolean);
    ...
    data.roleHierarchy || [];
    $$renderer2.push(`<div class="flex justify-center"><div class="card w-full max-w-lg bg-base-100 shadow-xl"><div class="card-body"><h1 class="card-title mb-6 text-2xl">Edit User: <span class="text-primary">${escape_html(data.userToEdit.username)}</span></h1> `);
    if (form?.message) {
      $$renderer2.push("<!--[-->");
    ...
    $$renderer2.push(`<!--]--> <form method="POST" class="space-y-4"><div class="form-control"><label class="label" for="username"><span class="label-text">Username</span></label> <input type="text" id="username" name="username"${attr("readonly", true, true)} required class="input input-bordered w-full"${attr("value", username)}/></div> <div class="form-control">`);
    {
      $$renderer2.push("<!--[!-->");
    }
    $$renderer2.push(`<!--]--> <button type="button" class="btn btn-secondary btn-sm mb-2">Reset Password</button></div> <div class="form-control"><div class="label"><span class="label-text">Roles</span></div> <div class="flex min-h-12 flex-wrap items-center gap-2 rounded-lg bg-base-200 p-2">`);
    ...
    $$renderer2.push(`<!--]--> Update User</button></div></form></div></div></div> `);
    {
      $$renderer2.push("<!--[!-->");
    }
    $$renderer2.push(`<!--]-->`);
